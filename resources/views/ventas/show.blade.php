        ...
        <div class="max-w-xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <h2 class="text-2xl font-semibold leading-tight mb-4">Ver Venta</h2>
                    <form method="POST" action="{{ route('ventas.update', $venta->id) }}">
                        @csrf
                        @method('PUT')
                        <div class="mb-4">
                            <x-input-label for="producto_id" :value="__('Producto')" />
                            <select id="producto_id" name="producto_id" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm"  disabled>
                        ...
                        <div class="mb-4">
                            ...
                            <select id="cliente_id" name="cliente_id" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" disabled>
                        ...
                        <div class="mb-4">
                            ...
                            <x-text-input id="fecha_venta" class="block mt-1 w-full" type="date" name="fecha_venta" :value="old('fecha_venta', $venta->fecha_venta)"  disabled/>
                            <x-input-error :messages="$errors->get('fecha_venta')" class="mt-2" />
                        </div>
                        <div class="mb-4">
                            <x-input-label for="subtotal" :value="__('Subtotal')" />
                            <x-text-input id="subtotal" class="block mt-1 w-full" type="number" step="0.01" name="subtotal" :value="old('subtotal')" :value="old('subtotal', $venta->subtotal)"  disabled />
                            <x-input-error :messages="$errors->get('subtotal')" class="mt-2" />
                        </div>
                        <div class="mb-4">
                            <x-input-label for="iva" :value="__('IVA')" />
                            <x-text-input id="iva" class="block mt-1 w-full" type="number" step="0.01" name="iva" :value="old('iva')" :value="old('iva', $venta->iva)"  disabled />
                            <x-input-error :messages="$errors->get('iva')" class="mt-2" />
                        </div>
                        <div class="mb-4">
                            <x-input-label for="total" :value="__('Total')" />
                            <x-text-input id="total" class="block mt-1 w-full" type="number" step="0.01" name="total" :value="old('total')" :value="old('total', $venta->total)"  disabled />
