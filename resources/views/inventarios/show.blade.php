        ...
        <div class="max-w-xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <h2 class="text-2xl font-semibold leading-tight mb-4">Ver entrada de inventario</h2>
                    <form method="POST" action="{{ route('inventarios.update', $inventario->id) }}">
                        ...
                        <div class="mb-4">
                            ...
                            <x-text-input id="fecha_entrada" class="block mt-1 w-full" type="date" name="fecha_entrada" :value="old('fecha_entrada', $inventario->fecha_entrada)"  disabled/>
                            <x-input-error :messages="$errors->get('fecha_entrada')" class="mt-2" />
                        </div>
                        <div class="mb-4">
                            <x-input-label for="fecha_salida" :value="__('Fecha de Salida')" />
                            <x-text-input id="fecha_salida" class="block mt-1 w-full" type="date" name="fecha_salida" :value="old('fecha_salida', $inventario->fecha_salida)"  disabled/>
                            <x-input-error :messages="$errors->get('fecha_salida')" class="mt-2" />
                        </div>
                        <div class="mb-4">
                            <x-input-label for="movimiento" :value="__('Movimiento')" />
                            <select id="movimiento" name="movimiento" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" disabled>
